...
async def lifespan(app: FastAPI):
    ...
    try:
        logger.info("Loading reranker model: %s", settings.RERANKER_MODEL_PATH)
        app.state.reranker = CrossEncoder(
            str(settings.RERANKER_MODEL_PATH),
            max_length=512 # Truncate chunks if they are too long for the reranker
        )
        logger.info("Reranker model loaded successfully.")
    except Exception as e:
        logger.exception("FATAL: Failed to initialize reranker: %s", e)
    ...
    # (We can add cleanup logic here if needed, e.g., closing DB connections)
    app.state.llama = None
    app.state.embedding = None
    app.state.reranker = None
...
app = FastAPI(title="Offline Code Assistant", lifespan=lifespan)
...
def _load_index_data() -> Tuple[Optional[faiss.Index], Optional[List[str]]]:
    ...
    """
    if not hasattr(app, "state") or app.state.embedding is None:
...
async def check_models_or_raise():
    ...
    async with app.state.reload_lock:
        ...
        if app.state.reranker is None:
            raise HTTPException(status_code=500, detail="Reranker model not loaded on server.")
...
async def retrieve_contexts(query: str, final_top_k: int = 5) -> List[Dict[str, Any]]:
    """
    Retrieve top-k code chunks for a given query.
    1. Search Faiss for RETRIEVAL_TOP_K chunk IDs.
    2. Fetch chunk text from SQLite.
    3. Rerank the chunks using the CrossEncoder.
    4. Return the new top-k chunks.
    """
    context_ids = []

    # 1. Embed query and search Faiss for chunk IDs
    async with app.state.reload_lock:
        if app.state.index is None or app.state.id_map is None or \
           app.state.embedding is None or app.state.reranker is None:
            logger.error("Retrieval attempted but a model is not loaded.")
            raise HTTPException(status_code=503, detail="Vector index or model not available.")

        # Embed query
        normalized_query = normalize_whitespace(query)
        qvec = app.state.embedding.embed([normalized_query])[0]

        # Search Faiss for candidate IDs
        results = app.state.embedding.search(
            app.state.index,
            app.state.id_map,
            qvec.reshape(1, -1),
    ...
    try:
        ...
        # Re-build contexts list in the correct order from Faiss
        for r_id in context_ids:
            if r_id in db_results:
                candidate_contexts.append(db_results[r_id])
    ...
    try:
        ...
        rerank_pairs = [(normalized_query, c['chunk']) for c in candidate_contexts]

        # Run blocking reranker prediction in a thread
        loop = asyncio.get_running_loop()
        scores = await loop.run_in_executor(
            None, # Use default thread pool
            app.state.reranker.predict,
            rerank_pairs
        )

        # Combine scores with their contexts
        scored_contexts = list(zip(scores, candidate_contexts))

        # Sort by score (highest score is most relevant)
        scored_contexts.sort(key=lambda x: x[0], reverse=True)

        # 4. Return the new top-k chunks
        reranked_contexts = [context for score, context in scored_contexts[:final_top_k]]

        return reranked_contexts

    except Exception as e:
        logger.exception("Failed to rerank contexts: %s", e)
        # Fallback: return the original top-k from vector search
        return candidate_contexts[:final_top_k]
...
async def read_index():
    # This returns your chat app's HTML file
...
async def query_endpoint(req: QueryRequest):
    ...
    # Use the new config setting for the *final* top_k
...
# ---------------------------------------------------------------------
# ADDED THIS BLOCK TO MAKE THE FILE RUNNABLE
# ---------------------------------------------------------------------
